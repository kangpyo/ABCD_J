...
function simulate!(sys, sim::ABCSimulator; n_threads::Integer=Threads.nthreads(), frozen_atoms=[], run_loggers=true, fname="output.txt")
    ...
    for step_n in 1:sim.max_steps
        ...
        for i in 1:length(sys.coords)
            ...
            sys.coords[i] += 1e-7*u"nm" * random_direction*d_multiplier[i]
        end
...
simulator = ABCSimulator(sigma=5e-2*u"nm", W=5e-1*u"eV", max_steps=500, max_steps_minimize=60, step_size_minimize=1e-3u"nm", tol=1e-12u"kg*m*s^-2")
# Run the simulation
...
visualize(molly_system.loggers.coords, boundary_condition, "sim_constrain.mp4", markersize=0.1, color=colors)